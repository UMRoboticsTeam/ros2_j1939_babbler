...
GenericCanDriver::Impl::Impl(rclcpp::Node* node)
: node_{node}
{
    ...
    msg_filter_ids_ = node_->declare_parameter<std::vector<int64_t>>("msg_filter_ids", {0}, msg_filter_range); // TODO: Explain somewhere that default is all-pass
    msg_filter_masks_ = node_->declare_parameter<std::vector<int64_t>>("msg_filter_masks", {0}, msg_filter_range);

    auto msg_topic_prefix = node_->declare_parameter<std::string>("msg_topic_prefix", "");
    ...
    RCLCPP_DEBUG(node_->get_logger(), "Generic Can Driver configured!");
}
...
void GenericCanDriver::Impl::rxFrame(const can_msgs::msg::Frame::SharedPtr& MSG)
{
  ...
  if(!MSG->is_rtr && !MSG->is_error && (device_ID_ == (MSG->id & 0x000000FFu) && filter(MSG->id)))
  {
    ...
    const can_msgs::msg::Frame::SharedPtr incoming_MSG = MSG;
      RCLCPP_DEBUG(node_->get_logger(), "Filtering message; sa:%d, count:%zu",MSG->id & 0x00FFFF00u, dbc_id_msg_map_.count(MSG->id & 0x00FFFF00u));
    ...
    if(dbc_id_msg_map_.count(MSG->id & 0x00FFFF00u) )
    {
      ...
      for (auto [key_signal, value_signal] : signals_map)
      {
        ...
        switch (value_signal.GetDataType()) {
            case NewEagle::INT:
                RCLCPP_DEBUG(node_->get_logger(), "Processing integer field: signed:%s, raw:%f, scale:%f, length:%u, offset:%f, result:%f", value_signal.GetSign() == NewEagle::SIGNED ? "Y" : "N", value_signal.GetInitialValue(), value_signal.GetGain(), value_signal.GetDlc(), value_signal.GetOffset(), value_signal.GetResult());
            ...
            case NewEagle::FLOAT:
               ...
               RCLCPP_DEBUG(node_->get_logger(), "Processing float field: raw:%f, scale:%f, offset:%f, result:%f", value_signal.GetInitialValue(), value_signal.GetGain(), value_signal.GetOffset(), value_signal.GetResult());
               break;
            case NewEagle::DOUBLE:
               can_data[dbc_signal_name_to_ros(key_signal)] = value_signal.GetResult();
               RCLCPP_DEBUG(node_->get_logger(), "Processing double field: raw:%f, scale:%f, offset:%f, result:%f", value_signal.GetInitialValue(), value_signal.GetGain(), value_signal.GetOffset(), value_signal.GetResult());
               break;
        }
      }
...
void GenericCanDriver::Impl::setupDatabase()
{
  ...
  for (auto [key, value] : dbc_name_msg_map_)
  {
    ...
    RCLCPP_DEBUG(node_->get_logger(), "Accepting messages with stripped ID:%d from raw ID:%d", stripped_id, value.GetId());
  }
}
...
void GenericCanDriver::Impl::configurePublishers(const std::string& msg_topic_prefix)
{
  ...
  for (auto [key_message, value_message] : dbc_name_msg_map_)
  {
      ...
      try {
        ...
        publishers_[key_message] = this->fish_->create_publisher(*node_, topic_name, msg_name, 20, rclcpp::PublisherOptions{});
    }
    catch (class_loader::LibraryLoadException& e) {
        RCLCPP_FATAL_STREAM(node_->get_logger(), "Failed to load library containing message type '" << msg_name << "'\n" << e.what());
        throw;
    }
    catch (ros_babel_fish::BabelFishException& e){
        RCLCPP_WARN_STREAM(node_->get_logger(), "Could not find message type for message '" << msg_name << "'\n" << e.what());
    }
  }
}

void GenericCanDriver::Impl::createDataArray(
...
void GenericCanDriver::Impl::generateAddressClaimAttackMsg(
...
bool GenericCanDriver::Impl::filter(const uint32_t id) const {
    bool pass = false;
