...
namespace ros2_j1939_babbler {
    ...
    void BabelBridge::Impl::rxFrame(const can_msgs::msg::Frame::SharedPtr& MSG) {
        ...
        if (!MSG->is_rtr && !MSG->is_error && (device_ID_ == (MSG->id & 0x000000FFu) && filter(MSG->id))) {
            ...
            if (dbc_id_msg_map_.count(MSG->id & 0x00FFFF00u)) {
                ...
                ros_babel_fish::CompoundMessage::SharedPtr can_data_ptr = fish_->create_message_shared(
                    msg_package_ + "/msg/" + dbc_message_name_to_ros(message.GetName()));
                    ...
                    switch (value_signal.GetDataType()) {
                        case NewEagle::INT:
                            RCLCPP_DEBUG(node_->get_logger(),
                                         ...
                                         value_signal.GetSign() == NewEagle::SIGNED ? "Y" : "N",
                                         value_signal.GetInitialValue(), value_signal.GetGain(), value_signal.GetDlc(),
                                         value_signal.GetOffset(), value_signal.GetResult());
                        // This is unbelievably ugly, but was only way I could get the compiler to not promote to int/uint and cause a Babel fish warning
                            switch (ceil_bits(value_signal.GetDlc())) {
                                case IntegerLengths::b8: if (value_signal.GetSign() == NewEagle::SIGNED) {
                                        can_data[dbc_signal_name_to_ros(key_signal)] = static_cast<int8_t>(value_signal.
                                            GetResult());
                                    } else {
                                        can_data[dbc_signal_name_to_ros(key_signal)] = static_cast<uint8_t>(value_signal.
                                            GetResult());
                                    }
                                    break;
                                case IntegerLengths::b16: if (value_signal.GetSign() == NewEagle::SIGNED) {
                                        can_data[dbc_signal_name_to_ros(key_signal)] = static_cast<int16_t>(value_signal.
                                            GetResult());
                                    } else {
                                        can_data[dbc_signal_name_to_ros(key_signal)] = static_cast<uint16_t>(value_signal.
                                            GetResult());
                                    }
                                    break;
                                case IntegerLengths::b32: if (value_signal.GetSign() == NewEagle::SIGNED) {
                                        can_data[dbc_signal_name_to_ros(key_signal)] = static_cast<int32_t>(value_signal.
                                            GetResult());
                                    } else {
                                        can_data[dbc_signal_name_to_ros(key_signal)] = static_cast<uint32_t>(value_signal.
                                            GetResult());
                                    }
                                    break;
                                case IntegerLengths::b64: if (value_signal.GetSign() == NewEagle::SIGNED) {
                                        can_data[dbc_signal_name_to_ros(key_signal)] = static_cast<int64_t>(value_signal.
                                            GetResult());
                                    } else {
                                        can_data[dbc_signal_name_to_ros(key_signal)] = static_cast<uint64_t>(value_signal.
                                            GetResult());
                                    }
                                    break;
                            }
                        ...
                        case NewEagle::FLOAT:
                            ...
                            RCLCPP_DEBUG(node_->get_logger(),
                                         "Processing float field: raw:%f, scale:%f, offset:%f, result:%f",
                                         value_signal.GetInitialValue(), value_signal.GetGain(), value_signal.GetOffset(),
                                         value_signal.GetResult());
                            break;
                        case NewEagle::DOUBLE: can_data[dbc_signal_name_to_ros(key_signal)] = value_signal.GetResult();
                            RCLCPP_DEBUG(node_->get_logger(),
                                         "Processing double field: raw:%f, scale:%f, offset:%f, result:%f",
                                         value_signal.GetInitialValue(), value_signal.GetGain(), value_signal.GetOffset(),
                                         value_signal.GetResult());
                            break;
                    }
    ...
    void BabelBridge::Impl::configurePublishers(const std::string& msg_topic_prefix) {
        ...
        for (auto [key_message, value_message] : dbc_name_msg_map_) {
            std::string msg_name = (std::ostringstream{} << msg_package_ << "/msg/" << dbc_message_name_to_ros(key_message)).
                    str();
            ...
            try {
                ...
                publishers_[key_message] = this->fish_->
                                                 create_publisher(*node_, topic_name, msg_name, 20,
                                                                  rclcpp::PublisherOptions{});
            } catch (class_loader::LibraryLoadException& e) {
                RCLCPP_FATAL_STREAM(node_->get_logger(),
                                    "Failed to load library containing message type '" << msg_name << "'\n" << e.what());
                throw;
            }
            catch (ros_babel_fish::BabelFishException& e) {
                RCLCPP_WARN_STREAM(node_->get_logger(),
                                   "Could not find message type for message '" << msg_name << "'\n" << e.what());
            }
        }
    }
