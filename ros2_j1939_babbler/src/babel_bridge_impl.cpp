...
namespace {
    ...
    template<typename T>
    void decodeAndPut(
            ros_babel_fish::Message& ros_signal, uint64_t raw_value, const ros2_j1939_babbler::PhysicalValue& can_signal,
            const rclcpp::Logger& logger
    );

    template<typename T>
    void encodeAndPut(
            const ros_babel_fish::Message& ros_signal, const ros2_j1939_babbler::PhysicalValue& can_signal,
            std::array<uint8_t, 8> can_data, const rclcpp::Logger& logger
    );
...
namespace ros2_j1939_babbler {
    ...
    void BabelBridge::Impl::on_can_to_ros(std::unique_ptr<can_msgs::msg::Frame> message) {
        ...
        if (!message->is_rtr && !message->is_error && filter(message->id)) {
            ...
            RCLCPP_DEBUG(node_->get_logger(), "Message passed filter; found in map: %s", found ? "true" : "false");
    ...
    void BabelBridge::Impl::on_ros_to_can(std::unique_ptr<ros_babel_fish::CompoundMessage> message) {
        ...
        if (!pgn_found) {
            RCLCPP_ERROR(
                    node_->get_logger(), "Could not map ROS message '%s' to message ID in DBC", message->name().c_str()
            );
            return;
        }
        ...
        for (const auto& signal_info : message_definition.signals | std::views::values) {
            putSignalInCanMessage(
                    (*message)[dbc_signal_name_to_ros(signal_info.signal.name())], can_message->data, signal_info,
                    node_->get_logger()
            );
        }
            ...
            try {
                ...
                std::string topic_name =
                        (std::ostringstream{} << msg_topic_prefix
                                ...
                                .str();
                publishers_[pgn] = fish_->create_publisher(*node_, topic_name, message_name, 20, rclcpp::PublisherOptions{});
                ...
                RCLCPP_FATAL_STREAM(
                        node_->get_logger(), "Failed to load library containing message type '" << message_name << "'\n"
                                                                                                << e.what()
                );
                throw;
            } catch (ros_babel_fish::BabelFishException& e) {
                RCLCPP_WARN_STREAM(
                        node_->get_logger(), "Could not find message type for message '" << message_name << "'\n"
                                                                                         << e.what()
                );
            }
        }
     ...
     */
    void tag_invoke(
            can::def_bo_cpo, ros2_j1939_babbler::DbcDatabase& this_, uint32_t msg_id, std::string msg_name, size_t msg_size,
     ...
     */
    void tag_invoke(
            can::def_sg_cpo, ros2_j1939_babbler::DbcDatabase& this_, uint32_t message_id,
        ...
        if (!this_.messages.contains(message_id)) {
            throw std::runtime_error((std::ostringstream{} << "Signal must not be defined before message\n"
        ...
        ros2_j1939_babbler::PhysicalValue value{ .codec = codec,
     ...
     */
    void tag_invoke(
            can::def_sig_valtype_cpo, ros2_j1939_babbler::DbcDatabase& this_, unsigned message_id, std::string sg_name,
            unsigned sg_ext_val_type
    ) {
        message_id = message_id & PGN_MASK;
        if (!this_.messages.contains(message_id)) {
            throw std::runtime_error((std::ostringstream{} << "Signal value type must not be defined before message\n"
                                                           << "Message ID: 0x" << std::hex << std::to_string(message_id)
                                                           << "\n"
                                                           << "Signal: '" << sg_name << "'")
                                             .str());
        ...
        if (!this_.messages.at(message_id).signals.contains(sg_name)) {
            throw std::runtime_error((std::ostringstream{} << "Signal value type must not be defined before signal itself\n"
                                                           << "Message ID: 0x" << std::hex << std::to_string(message_id)
                                                           << "\n"
                                                           << "Signal: '" << sg_name << "'")
                                             .str());
...
namespace {
    ...
    void putSignalInRosMessage(
        ...
        switch (can_signal.signal.value_type()) {
            case can::i64:
                RCLCPP_DEBUG_STREAM(
                        logger, "Processing signed integer signal '"
                                        << can_signal.signal.name() << "': raw=" << raw << ", scale=" << can_signal.factor
                                        << ", offset=" << can_signal.value_offset << ", length=" << can_signal.size
                );
                switch (ceil_bits(can_signal.size)) {
                    case IntegerLengths::b8: decodeAndPut<int8_t>(ros_signal, raw, can_signal, logger); break;
                    case IntegerLengths::b16: decodeAndPut<int16_t>(ros_signal, raw, can_signal, logger); break;
                    case IntegerLengths::b32: decodeAndPut<int32_t>(ros_signal, raw, can_signal, logger); break;
                    case IntegerLengths::b64: decodeAndPut<int64_t>(ros_signal, raw, can_signal, logger); break;
                }
                break;
            case can::u64:
                RCLCPP_DEBUG_STREAM(
                        logger, "Processing unsigned integer signal '"
                                        << can_signal.signal.name() << "': raw=" << raw << ", scale=" << can_signal.factor
                                        << ", offset=" << can_signal.value_offset << ", length=" << can_signal.size
                );
                ...
                switch (ceil_bits(can_signal.size)) {
                    case IntegerLengths::b8: decodeAndPut<uint8_t>(ros_signal, raw, can_signal, logger); break;
                    case IntegerLengths::b16: decodeAndPut<uint16_t>(ros_signal, raw, can_signal, logger); break;
                    case IntegerLengths::b32: decodeAndPut<uint32_t>(ros_signal, raw, can_signal, logger); break;
                    case IntegerLengths::b64: decodeAndPut<uint64_t>(ros_signal, raw, can_signal, logger); break;
                }
                break;
            case can::f32:
                RCLCPP_DEBUG_STREAM(
                        logger, "Processing float signal '"
                                        << can_signal.signal.name() << "': raw=" << raw << ", scale=" << can_signal.factor
                                        << ", offset=" << can_signal.value_offset << ", length=" << can_signal.size
                );
            ...
            case can::f64:
                RCLCPP_DEBUG_STREAM(
                        logger, "Processing double signal '"
                                        << can_signal.signal.name() << "': raw=" << raw << ", scale=" << can_signal.factor
                                        << ", offset=" << can_signal.value_offset << ", length=" << can_signal.size
                );
    ...
    void putSignalInCanMessage(
        ...
        switch (can_signal.signal.value_type()) {
            case can::i64:
                switch (ceil_bits(can_signal.size)) {
                    case IntegerLengths::b8: encodeAndPut<int8_t>(ros_signal, can_signal, can_data, logger); break;
                    case IntegerLengths::b16: encodeAndPut<int16_t>(ros_signal, can_signal, can_data, logger); break;
                    case IntegerLengths::b32: encodeAndPut<int32_t>(ros_signal, can_signal, can_data, logger); break;
                    case IntegerLengths::b64: encodeAndPut<int64_t>(ros_signal, can_signal, can_data, logger); break;
                }
                break;
            case can::u64:
                switch (ceil_bits(can_signal.size)) {
                    case IntegerLengths::b8: encodeAndPut<uint8_t>(ros_signal, can_signal, can_data, logger); break;
                    case IntegerLengths::b16: encodeAndPut<uint16_t>(ros_signal, can_signal, can_data, logger); break;
                    case IntegerLengths::b32: encodeAndPut<uint32_t>(ros_signal, can_signal, can_data, logger); break;
                    case IntegerLengths::b64: encodeAndPut<uint64_t>(ros_signal, can_signal, can_data, logger); break;
                }
                RCLCPP_DEBUG_STREAM(
                        logger, "Pushed unsigned integer signal '"
                                        << can_signal.signal.name() << "': scale=" << can_signal.factor
                                        << ", offset=" << can_signal.value_offset << ", length=" << can_signal.size
                );
                break;
            case can::f32:
                ...
                RCLCPP_DEBUG_STREAM(
                        logger, "Pushed float signal '" << can_signal.signal.name() << "': scale=" << can_signal.factor
                                                        << ", offset=" << can_signal.value_offset
                                                        << ", length=" << can_signal.size
                );
                break;
            case can::f64:
                encodeAndPut<double>(ros_signal, can_signal, can_data, logger);
                RCLCPP_DEBUG_STREAM(
                        logger, "Pushed double signal '" << can_signal.signal.name() << "': scale=" << can_signal.factor
                                                         << ", offset=" << can_signal.value_offset
                                                         << ", length=" << can_signal.size
                );
                break;
    ...
    void decodeAndPut(
        ...
        if (data < can_signal.min || data > can_signal.max) {
            RCLCPP_DEBUG_STREAM(
                    logger, "Signal '" << can_signal.signal.name() << "': value=" << data
                                       << " is out of range; min=" << can_signal.min << ", max=" << can_signal.max
            );
        }
        ros_signal = data;
    }
    ...
    void encodeAndPut(
        ...
        if (value < can_signal.min || value > can_signal.max) {
            RCLCPP_DEBUG_STREAM(
                    logger, "Signal '" << can_signal.signal.name() << "': value=" << value
                                       << " is out of range; min=" << can_signal.min << ", max=" << can_signal.max
            );
        }
        uint64_t raw_value =
                (ros_signal.as<ros_babel_fish::ValueMessage<T>>().getValue() - can_signal.value_offset) / can_signal.factor;
