...
## DBC Intricacies
By default, the bridge will match messages in the J1939 style. This means that for example having a DBC file with
both `0x12345600` and `0x123456AA` messages yields undefined behaviour, as those correspond to the same J1939 PGN.
If you wish to match a non-J1939 message, you must specify the `VFrameFormat` attribute for that message. This can be
done through the `BA_ "VFrameFormat" BO_ ...` command. The `BatteryVoltage` and `BatteryTemp` messages in the sample
DBC file are an example of how to do this. A default format can be set with `BA_DEF_DEF_`, but in the absence of this
the default is considered to be J1939PG.

At this time standard format messages are not supported. If any of these are present in the DBC, a runtime error will be
generated by the babel bridge or a compile-time error by the static bridge
